...
1. Bash ツールで `mkdir -p docs/codex-plan-review` を実行し、作業ディレクトリを確保する
2. Write ツールで `docs/codex-plan-review/schema.json` に以下の JSON Schema を書き出す:
...
3. Write ツールで `docs/codex-plan-review/prompt.md` に以下のプロンプトを書き出す:
...
4. Bash ツールで以下を実行する（`timeout: 300000`（5分）を指定すること。Codex CLI のレビューは120秒を超えることがあり、デフォルトタイムアウトではバックグラウンドに移行してしまう）:
```bash
cat docs/codex-plan-review/prompt.md | codex exec --output-schema docs/codex-plan-review/schema.json -
```

5. 出力を JSON としてパースする
   - **注意:** Codex CLI の出力にはヘッダ（バージョン情報等）とプロンプトエコーが含まれる。出力の末尾にある JSON オブジェクト（`{` で始まる行以降）を抽出してパースすること
   - パース失敗の場合: 1回だけ再試行する（同じコマンドを再実行）
   - 再試行でもパース失敗の場合: 作業ファイルを削除（ステップ 6 のクリーンアップ）してから、出力をそのままユーザーに表示してフローを中断する

6. Codex CLI がエラー（非ゼロ終了コード）を返した場合は、作業ファイルを削除（ステップ 6 のクリーンアップ）してから、エラー内容をユーザーに報告してフローを中断する
...
- ループ回数をカウントする（最大5回）
- **`docs/codex-plan-review/prompt.md` を新しい内容で上書きする**。以下を含める:
  ...
  4. PBI の内容（ある場合）
- 更新したプロンプトファイルで codex exec を再実行する（ステップ 3 の手順 4〜6 と同様）
...
1. Bash ツールで作業ファイルを削除する:
```bash
rm -rf docs/codex-plan-review
```
