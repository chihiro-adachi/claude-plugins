...
description: This skill should be used when the user explicitly mentions Codex in the context of plan review, such as "review plan with codex", "planをCodexでレビュー", "計画をCodexでレビュー", "Codexにレビューさせて", "codexでチェック". Only trigger when the user's request specifically references both a plan (plan/計画/実装計画) AND Codex. Do NOT trigger for generic "レビューして" or "review it" requests without explicit Codex mention, as those likely refer to code review, not this skill.
---
...
3. **`threadId` の記録:** `mcp__codex__codex` のツール応答には、レビュー内容（JSON テキスト）とは別にメタデータとして `threadId` が含まれる。この値を記録しておく。ステップ 4 の再レビューで `mcp__codex__codex-reply` に渡すために必要。`threadId` が取得できなかった場合は、ステップ 4 で `mcp__codex__codex-reply` の代わりに `mcp__codex__codex` を新規呼び出しする（単発再実行フォールバック）

### ステップ 3: 指摘の評価と修正

1. `overview.verdict` が `"LGTM"` → ステップ 5 へ
2. `overview.verdict` が `"BLOCKED"` → ブロッカーをユーザーに報告し、フローを中断する。ユーザーが計画を修正して再度スキルを実行するまで再レビューしない
3. `requirement_questions` がある場合 → ユーザーに確認事項として提示し、回答を待つ。回答が得られるまで findings の修正には進まない（未解決の前提で plan を変更しない）。回答後の処理:
   - ユーザーの回答内容を plan ファイルの該当箇所に「前提・補足」として Edit で追記する
   - 回答によって findings の妥当性が変わる場合があるため、回答を踏まえた上で手順 4 の評価に進む
4. `findings` 配列の各指摘を評価する:
   - 技術的に妥当か
   - plan のコンテキストに合っているか
   - severity と impact の妥当性
   - **重複判定:** 過去のラウンドで却下済みの指摘と同一論点かを判定する。`affected_step` と `category` が一致し、`problem` の趣旨が同じであれば、id や文言が異なっても同一論点とみなし自動却下する
5. 妥当と判断した指摘のみ、Edit ツールで plan ファイルを修正する。以下の編集ルールに従うこと:
   - 修正は finding の `affected_step` が示す箇所に限定する。無関係なステップを変更しない
   - 既存ステップの削除・順序変更は行わない。追記・文言修正のみ許可
   - 1つの finding につき1回の Edit で完結させ、複数 finding をまとめて1回の Edit にしない
...
上記テンプレートから prompt を構築し、以下のいずれかで呼び出す:

- **threadId あり（通常）:** `mcp__codex__codex-reply({ threadId: <記録した threadId>, prompt: <構築した文字列> })` で同一セッションを継続
- **threadId なし（フォールバック）:** `mcp__codex__codex({ prompt: <ステップ 2 の初回プロンプトテンプレートに却下指摘の除外指示を追加した文字列> })` で新規セッションとして実行。この場合、初回と同じレビュー観点・出力形式の全文をプロンプトに含めること
...
- `"BLOCKED"` → ステップ 3 の BLOCKED 処理と同様、ユーザーに報告しフローを中断する
- **停滞検出:** 新規の findings がすべて過去ラウンドの却下済み指摘と同一論点（重複判定による自動却下）だった場合、実質進展なしと判断しステップ 5 へ進む。2回連続で進展なしの場合も同様に終了する
- 5回到達で `"LGTM"` にならなければ → ステップ 5 へ
- **重要:** 新規の指摘がある限り、`overview.verdict` が `"LGTM"` になるまでループを継続すること。独自判断で打ち切らないこと
